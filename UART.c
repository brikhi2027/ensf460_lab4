...
void DispTesting(unsigned int DispData)   
{
    ...
    int x;
    for (x = 0; x < 25; x++) {
        XmitUART2(' ',1);  // Clear Gap
    }
    XmitUART2('\r',1);  // return Gap
    return;
}

void DispAsterisks(uint16_t ADC_value)
{
    uint16_t num_asterisks = ((32 * ADC_value) / 1023) + 1;
    Disp2String("Mode 0: ");
    int x;
    for (x = 0; x < num_asterisks; x++){
        Disp2String("*");
    }
    DispTesting(ADC_value);
    return;
}

void DispMode1(uint16_t value){
    XmitUART2(' ',1);  // Disp Gap
